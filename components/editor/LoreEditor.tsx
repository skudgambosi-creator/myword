...
export default function LoreEditor({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error } = await lore.storage.from('yarn-images').upload(fileName, file, { contentType: file.type })
    if (error) {
      setUploadError(`Image upload failed: ${error.message}`)
    } else {
      const url = `${process.env.NEXT_PUBLIC_LORE_SUPABASE_URL}/storage/v1/object/public/yarn-images/${fileName}`
  ...
  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const safeName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, '_')
    const fileName = `${userId}/${Date.now()}-${safeName}`
    const { error } = await lore.storage.from('yarn-audio').upload(fileName, file, { contentType: file.type })
    if (error) {
      setUploadError(`Audio upload failed: ${error.message}`)
    } else {
      const url = `${process.env.NEXT_PUBLIC_LORE_SUPABASE_URL}/storage/v1/object/public/yarn-audio/${fileName}`
  ...
  return (
    <div>
      ...
      <div className="tiptap-toolbar">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().unsetColor().run()} isActive={!editor.isActive('textStyle', { color: '#CC0000' })} title="Black text"><span style={{ color: '#000' }}>A</span></ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().setColor('#CC0000').run()} isActive={editor.isActive('textStyle', { color: '#CC0000' })} title="Red text"><span style={{ color: '#CC0000' }}>A</span></ToolbarButton>
        <span style={{ borderLeft: '1px solid #ccc', margin: '0 4px' }} />
        ...
        <ToolbarButton onClick={() => editor.chain().focus().setTextAlign('center').run()} isActive={editor.isActive({ textAlign: 'center' })} title="Align centre">◉</ToolbarButton>
        ...
        <span style={{ borderLeft: '1px solid #ccc', margin: '0 4px' }} />
        <ToolbarButton onClick={() => fileInputRef.current?.click()} title="Insert image">IMG</ToolbarButton>
        ...
        <ToolbarButton onClick={() => audioInputRef.current?.click()} title="Insert audio">AUD</ToolbarButton>
        <input ref={audioInputRef} type="file" accept="audio/mpeg,audio/wav,audio/ogg,audio/mp4,audio/aac" onChange={handleAudioUpload} style={{ display: 'none' }} />
      </div>
